...
export class GoldStandardService {
  ...
  async captureApprovedValuation(
  ...
  async getCase(
    ...
    return this.gold.getCaseBundle(organizationId, caseId);
  }

  async listCases(organizationId: string): Promise<GoldStandardCase[]> {
    return this.gold.listCases(organizationId);
  }
  ...
  async assignPartition(
    ...
    if (goldCase.status !== "VALIDATED") {
      throw new PersistenceError(
        "GOLD_CASE_NOT_VALIDATED",
        "Only validated Gold Standard cases can be assigned to calibration or holdout partitions.",
      );
    }
  ...
  ): Promise<GoldStandardComparisonResult> {
...
function requireApprovedValuation(
  valuation: Valuation | null,
): asserts valuation is Valuation & { totalPoints: number; gradeCode: string } {
  if (valuation === null) {
    throw new PersistenceError(
      "VALUATION_NOT_FOUND",
  ...
  if (valuation.status !== "APPROVED") {
    throw new PersistenceError(
      "GOLD_SOURCE_NOT_APPROVED",
      `Gold Standard capture requires APPROVED status; received ${valuation.status}.`,
    );
  }
  if (valuation.totalPoints === null || valuation.gradeCode === null) {
    throw new PersistenceError(
      "GOLD_SOURCE_INCOMPLETE",
      "Approved valuation has no final points or grade.",
    );
  }
}

function snapshotJob(job: Job) {
  return {
    code: job.code,
    name: job.name,
