...
export async function parseGoldStandardSpreadsheet(
  ...
  for (const row of table.rows) {
    ...
    if (existing === undefined) {
      if (label === undefined) {
        failRow(row, "SPREADSHEET_REQUIRED_VALUE", "etiqueta_anonima es obligatoria en la primera fila del caso.", "etiqueta_anonima");
      }
      if (methodologyId === undefined) {
        failRow(row, "SPREADSHEET_REQUIRED_VALUE", "id_metodologia es obligatorio en la primera fila del caso.", "id_metodologia");
      }
      if (jobName === undefined) {
        failRow(row, "SPREADSHEET_REQUIRED_VALUE", "puesto es obligatorio en la primera fila del caso.", "puesto");
      }
      item = {
        caseCode,
        ...
        notes: undefined,
        decisions: new Map<string, DecisionAccumulator>(),
      };
      cases.set(caseCode, item);
    } else {
      item = existing;
      if (label !== undefined) {
        item.anonymizedLabel = mergeRequiredText(item.anonymizedLabel, label, row, "etiqueta_anonima");
      }
      if (methodologyId !== undefined) {
        item.methodologyVersionId = mergeRequiredText(item.methodologyVersionId, methodologyId, row, "id_metodologia");
      }
      if (jobName !== undefined) {
        item.jobName = mergeRequiredText(item.jobName, jobName, row, "puesto");
      }
    }
    ...
    item.expectedTotalPoints = mergeOptional(
      item.expectedTotalPoints,
      parseFiniteOptional(field(row, GOLD_ALIASES.expectedTotalPoints), row, "puntos_esperados"),
      row,
      "puntos_esperados",
    );
    item.expectedGradeCode = mergeOptional(
      item.expectedGradeCode,
      optionalText(row, GOLD_ALIASES.expectedGradeCode),
      row,
      "grado_esperado",
    );
    ...
    if (decision === undefined) {
      ...
      item.decisions.set(dimensionCode, created);
      decision = created;
    } else {
      if (decision.selectedLevelCode !== selectedLevelCode) {
        failRow(
          ...
          `La dimensión ${dimensionCode} repite niveles distintos (${decision.selectedLevelCode} y ${selectedLevelCode}).`,
          "codigo_nivel",
        );
      }
    ...
    if (evidenceExcerpt === undefined) {
      if (evidenceTypeRaw !== undefined || (evidenceSection !== undefined && evidenceSection !== null)) {
        failRow(
          row,
          "SPREADSHEET_EVIDENCE_INCOMPLETE",
          "tipo_evidencia/seccion_evidencia no pueden usarse sin evidencia.",
          "evidencia",
        );
      }
    } else {
      decision.evidence.push({
        sourceType: parseEvidenceSource(evidenceTypeRaw, row),
  ...
  if (cases.size === 0) {
    throw new SpreadsheetImportError("SPREADSHEET_EMPTY", "El archivo no contiene filas de datos.");
  }
  ...
  const document = {
    ...
    cases: [...cases.values()].map((item) => {
      ...
      const job: GoldStandardJobSnapshot = {
      ...
      return {
        ...
        decisions,
        ...(item.expectedTotalPoints === undefined ? {} : { expectedTotalPoints: item.expectedTotalPoints }),
        ...(item.expectedGradeCode === undefined ? {} : { expectedGradeCode: item.expectedGradeCode }),
        ...(item.partition === undefined ? {} : { partition: item.partition }),
        ...(item.isAnchor === undefined ? {} : { isAnchor: item.isAnchor }),
        expertUserId: null,
...
export async function parseMethodologySpreadsheet(
  ...
  for (const row of table.rows) {
    const recordType = normalizeRecordType(requiredText(row, METHOD_ALIASES.recordType, "tipo_registro"), row);
    switch (recordType) {
      case "META":
        if (metadataRow !== undefined) {
          failRow(row, "SPREADSHEET_DUPLICATE_META", "Solo puede existir una fila META.", "tipo_registro");
        }
        metadataRow = row;
        break;
      case "FACTOR":
  ...
  if (metadataRow === undefined) {
    throw new SpreadsheetImportError("SPREADSHEET_META_MISSING", "La metodología necesita una fila META.");
  }
  if (factorRows.length === 0) {
    throw new SpreadsheetImportError("SPREADSHEET_FACTORS_MISSING", "La metodología necesita al menos un FACTOR.");
  }
  if (stepRows.length === 0) {
    throw new SpreadsheetImportError("SPREADSHEET_STEPS_MISSING", "La metodología necesita al menos un STEP.");
  }
  if (gradeRows.length === 0) {
    throw new SpreadsheetImportError("SPREADSHEET_GRADES_MISSING", "La metodología necesita al menos un GRADE.");
  }

  const factorMap = new Map<string, FactorDefinition>();
  const factors = factorRows.map((row): FactorDefinition => {
    const code = requiredText(row, METHOD_ALIASES.code, "codigo");
    if (factorMap.has(code)) {
      failRow(row, "SPREADSHEET_DUPLICATE_CODE", `Factor duplicado: ${code}.`, "codigo");
    }
  ...
  for (const row of dimensionRows) {
    const code = requiredText(row, METHOD_ALIASES.code, "codigo");
    if (dimensionMap.has(code)) {
      failRow(row, "SPREADSHEET_DUPLICATE_CODE", `Dimensión duplicada: ${code}.`, "codigo");
    }
    ...
    if (factor === undefined) {
      failRow(
        ...
        `La dimensión ${code} referencia un factor inexistente: ${parent}.`,
        "codigo_padre",
      );
    }
    const description = optionalText(row, METHOD_ALIASES.description);
  ...
  for (const row of levelRows) {
    ...
    if (dimension === undefined) {
      failRow(row, "SPREADSHEET_PARENT_NOT_FOUND", `El nivel ${code} referencia una dimensión inexistente: ${parent}.`, "codigo_padre");
    }
    const key = `${parent}::${code}`;
    if (levelKeys.has(key)) {
      failRow(row, "SPREADSHEET_DUPLICATE_CODE", `Nivel duplicado ${code} en ${parent}.`, "codigo");
    }
    levelKeys.add(key);
    const description = optionalText(row, METHOD_ALIASES.description);
  ...
  const steps = stepRows.map((row): ScoringStep => {
    const code = requiredText(row, METHOD_ALIASES.code, "codigo");
    if (stepMap.has(code)) {
      failRow(row, "SPREADSHEET_DUPLICATE_CODE", `Paso duplicado: ${code}.`, "codigo");
    }
    ...
    switch (type) {
      case "lookup":
        step = {
          ...base,
          type,
          inputs: parseReferenceList(optionalText(row, METHOD_ALIASES.references), row, true) as LookupInputReference[],
          table: {},
        };
        break;
      case "sum":
      case "multiply":
        step = {
          ...base,
          type,
          operands: parseReferenceList(optionalText(row, METHOD_ALIASES.references), row, false) as NumericReference[],
        };
        break;
      case "divide":
        step = {
      ...
      case "round":
        step = {
          ...
          precision: parseFiniteRequired(field(row, METHOD_ALIASES.precision), row, "precision"),
        };
        break;
      default:
        failRow(
          row,
          "SPREADSHEET_INVALID_STEP_TYPE",
          `tipo_paso debe ser lookup, sum, multiply, divide o round; recibido ${type}.`,
          "tipo_paso",
        );
    }
  ...
  for (const row of lookupRows) {
    ...
    if (step === undefined) {
      failRow(row, "SPREADSHEET_PARENT_NOT_FOUND", `LOOKUP referencia un STEP inexistente: ${parent}.`, "codigo_padre");
    }
    if (step.type !== "lookup") {
      failRow(row, "SPREADSHEET_LOOKUP_PARENT_INVALID", `El STEP ${parent} no es de tipo lookup.`, "codigo_padre");
    }
    const key = requiredText(row, METHOD_ALIASES.lookupKey, "clave_lookup");
    if (Object.hasOwn(step.table, key)) {
      failRow(row, "SPREADSHEET_DUPLICATE_LOOKUP_KEY", `Clave lookup duplicada ${key} en ${parent}.`, "clave_lookup");
    }
  ...
  return parseMethodologyDefinition({
...
async function loadXlsx(bytes: Uint8Array, preferredSheet: string): Promise<TabularData> {
  ...
  if (workbook.worksheets.length === 0) {
    throw new SpreadsheetImportError("SPREADSHEET_NO_SHEETS", "El XLSX no contiene hojas.");
  }
  const worksheet =
    workbook.worksheets.find((sheet) => sheet.name.toLowerCase() === preferredSheet.toLowerCase()) ??
...
function excelCellValue(value: unknown, row: number, column: number): Scalar {
  ...
  if (typeof value === "object") {
    ...
    if (typeof object.text === "string") {
      return boundedString(object.text, row, String(column));
    }
  }
  throw new SpreadsheetImportError(
...
function parseDelimited(text: string, delimiter: string): string[][] {
  ...
  if (quoted) {
    throw new SpreadsheetImportError("SPREADSHEET_INVALID_CSV", "El CSV contiene una comilla sin cerrar.");
  }
  row.push(fieldValue);
  if (row.some((value) => value.trim() !== "")) rows.push(row);
  return rows;
...
function requiredText(row: TabularRow, aliases: readonly string[], label: string): string {
  ...
  if (value === undefined) {
    failRow(row, "SPREADSHEET_REQUIRED_VALUE", `${label} es obligatorio.`, label);
  }
  return value;
}
...
function mergeRequiredText(current: string, incoming: string, row: TabularRow, label: string): string {
  if (current !== incoming) {
    failRow(row, "SPREADSHEET_CONFLICT", `${label} cambia dentro del mismo caso (${current} vs ${incoming}).`, label);
  }
  return current;
}
...
function mergeOptional<T>(
  ...
  if (!sameValue(current, incoming)) {
    failRow(row, "SPREADSHEET_CONFLICT", `${label} cambia dentro del mismo caso.`, label);
  }
  return current;
}
...
function parseBooleanRequired(value: Scalar | undefined, row: TabularRow, label: string): boolean {
  ...
  const normalized = value === undefined || value === null
    ? ""
    : stripAccents(String(value).trim().toLowerCase());
...
function parseFiniteRequired(value: Scalar | undefined, row: TabularRow, label: string): number {
  ...
  if (!Number.isFinite(parsed)) {
    failRow(row, "SPREADSHEET_INVALID_NUMBER", `${label} debe ser numérico.`, label);
  }
  return parsed;
}
...
function parseEvidenceSource(value: string | undefined, row: TabularRow): EvidenceSourceType {
  if (value === undefined) {
    failRow(row, "SPREADSHEET_EVIDENCE_INCOMPLETE", "tipo_evidencia es obligatorio cuando existe evidencia.", "tipo_evidencia");
  }
  const normalized = stripAccents(value.trim().toUpperCase());
...
function normalizeRecordType(
  value: string,
  row: TabularRow,
): "META" | "FACTOR" | "DIMENSION" | "LEVEL" | "STEP" | "LOOKUP" | "GRADE" {
  const normalized = stripAccents(value.trim().toUpperCase());
  ...
  failRow(row, "SPREADSHEET_INVALID_RECORD_TYPE", `tipo_registro no reconocido: ${value}.`, "tipo_registro");
}
...
function parseReferenceList(
  ...
  row: TabularRow,
  lookup: boolean,
): Array<LookupInputReference | NumericReference> {
  ...
  return value
    ...
    .map((token) =>
      lookup
        ? parseLookupReferenceToken(token, row, "referencias")
        : parseNumericReferenceToken(token, row, "referencias"),
    );
}
...
function parseNumericReferenceToken(token: string, row: TabularRow, label: string): NumericReference {
  ...
  if (kind === "constant" && value !== "") {
    ...
    if (!Number.isFinite(parsed)) {
      failRow(row, "SPREADSHEET_INVALID_REFERENCE", `${label}: constante inválida ${value}.`, label);
    }
